...
#SBATCH --job-name=finetuned_baxter_on_demodata  # Job name
...
#SBATCH --output=batch_files/finetuned_baxter_on_demodata_output_%j.log      # Standard output log
# Note: The above comments are supposed to be like this, its how the compiler registers flags for sbatch

# TO RUN THIS SCRIPT DO: sbatch -A axb1653 -p gpu --constraint=gpul40s batch_files/run_finetuned_baxter_on_demodata.sh
...
python -u scripts/get_trajectory_mse.py -em new_embodiment -dc baxter_arms -d demo_data/robot_sim.PickNPlace -ckpt models/finetuned-baxter-model -p Finetuned-Baxter-Policy-On-Demodata
conda deactivate
